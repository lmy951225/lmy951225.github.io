...
void DynamicsController_Start(void)
{
  ...
  for (i = 0; i < 6; i++) {
    ...
    q[i] = ZeroForceDrag_MoreJoints_P._InitialValue[i];

    /* Start for DataStoreMemory: '<S1>/各个关节内转子采集 到的实际角速度' */
    dq[i] = ZeroForceDrag_MoreJoints_P._InitialValue_p[i];

    /* Start for DataStoreMemory: '<S1>/驱动各个关节 运动的力矩' */
    Tau[i] = ZeroForceDrag_MoreJoints_P._InitialValue_n[i];
  }
}
...
void DynamicsController(void)
{
  ...
  q[0] = ZeroForceDrag_MoreJoints_B.Saturation2;
  q[1] = ZeroForceDrag_MoreJoints_B.Saturation1;
  q[2] = ZeroForceDrag_MoreJoints_B.Saturation3;
  q[3] = ZeroForceDrag_MoreJoints_B.Saturation4;
  q[4] = ZeroForceDrag_MoreJoints_B.Saturation5;
  q[5] = ZeroForceDrag_MoreJoints_B.Saturation6;
   ...
   */
  for (i = 0; i < 6; i++) {
    Send_GUIdata[(int32_T)i] = ZeroForceDrag_MoreJoints_P.Gain_Gain_c * q[i];
  }

  /* End of DataStoreWrite: '<S1>/上位机监控窗口1~6——参数1' */

  /* DataStoreWrite: '<S1>/各关节角速度' */
  dq[0] = ZeroForceDrag_MoreJoints_B.Gain9_de;
  dq[1] = ZeroForceDrag_MoreJoints_B.Gain9_dw;
  dq[2] = ZeroForceDrag_MoreJoints_B.Gain9_m;
  dq[3] = ZeroForceDrag_MoreJoints_B.Gain9_d;
  dq[4] = ZeroForceDrag_MoreJoints_B.Gain9;
  dq[5] = ZeroForceDrag_MoreJoints_B.u;
  ...
  Tau[0L] = ZeroForceDrag_MoreJoints_B.Saturation3;
  Tau[1L] = ZeroForceDrag_MoreJoints_B.Saturation2;
  Tau[2L] = ZeroForceDrag_MoreJoints_B.Saturation4;
  Tau[3L] = ZeroForceDrag_MoreJoints_B.Saturation5;
  Tau[4L] = ZeroForceDrag_MoreJoints_B.Saturation1;
  Tau[5L] = ZeroForceDrag_MoreJoints_B.Saturation6;
  ...
  ZeroForceDrag_MoreJoints_B.u = J7[0];
